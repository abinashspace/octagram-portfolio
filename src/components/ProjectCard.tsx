...
export function ProjectCard({ project, delay = 0 }: ProjectCardProps) {
  return (
    <Reveal delay={delay} className="group h-full">
      <TiltCard maxTilt={5} liftOnHover={6} className="h-full">
        <Link
          to={`/work/${project.slug}`}
          className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface transition-colors duration-500 hover:border-accent-border hover:shadow-card"
        >
